 ...
 */
@TeleOp(name = "TankFinalClusterMagnetFuckDifferentFunctionCrap", group = "Rover")
public class roverTeleOpNoEncoderOneWay extends OpMode {
    ...
    public void init() {
        ...
        mp = MediaPlayer.create(this.hardwareMap.appContext,R.raw.ocean_man);
    ...
    public void loop(){
        ...
        if(inFlipTrigger2 && inFlipTimer.seconds() > 0.38){
            inFlipTrigger2 = false;
            robot.inFlip.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
            robot.inFlip.setPower(0);
        }

        if(inFlipTimer.seconds()>0.7){
            inFlipTrigger1=false;
            inFlipTrigger2 = false;
        }
